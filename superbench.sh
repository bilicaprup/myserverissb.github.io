	...
	if [[ "$GeekbenchTest" == "Y" ]]; then
		...
		if [[ $ARCH = *x86* ]]; then
			...
			if [ ! -e './geekbench/geekbench6' ]; then
				...
				if [[ "$IsGlobal" == "1" ]];then
					wget -qO- https://cdn.geekbench.com/k9ea2vqm/Geekbench-6.0.0-Linux.tar.gz | tar xz --strip-components=1 -C ./geekbench &>/dev/null
				else
					wget -qO- https://down.vpsaff.net/linux/geekbench/Geekbench-6.0.0-Linux.tar.gz  | tar xz --strip-components=1 -C ./geekbench &>/dev/null
				fi
			fi
